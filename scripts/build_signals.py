...
# Веса откалиброваны по бэктесту (docs/data/backtest.json): голая «пружина»
# показала edge ≈ 0 (даже отрицательный на 60д) — понижена. Инсайдеры (edge из
# исследований) и совпадения оставлены высокими; тренд/цикл даёт реальный edge
# через рыночную позу. Меняй под себя.
WEIGHTS = {
    "confluence":       5.0,   # пружина ↑ + инсайдер покупает (edge от инсайдера)
    "insider_cluster":  4.0,   # ≥2 инсайдера в одну сторону (документированный edge)
    "fund_consensus":   3.0,   # 2+ фонда двигают твою бумагу
    "macro_spike":      2.0,   # резкое движение сырья/ставок
    "spring_fired":     1.5,   # разжатие пружины — по бэктесту edge≈0, вес понижен
    "spring_loaded":    0.5,   # взведённая пружина — пре-сигнал, edge не доказан
}
